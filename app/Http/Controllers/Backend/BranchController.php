...
class BranchController extends Controller
{
    ...
    {
        Log::info('Branch store invoked', [
            'route' => request()->path(),
            'input' => $request->except(['feature_image'])
        ]);
        ...
        try {
            ...
            if (isset($data['address']) && is_string($data['address']) && $data['address'] !== '') {
                $decodedAddress = json_decode($data['address'], true);
                if (json_last_error() === JSON_ERROR_NONE && is_array($decodedAddress)) {
                    $allowed = ['address_line_1','address_line_2','postal_code','city','state','country','latitude','longitude','is_primary'];
                    $clean = Arr::only($decodedAddress, $allowed);
                    if (!empty($clean)) {
                        $query->address()->save(new Address($clean));
                    }
                }
            }
            ...
            Log::info('Branch store success', ['branch_id' => $branch_id]);
            ...
            return response()->json([
                'status' => false,
                'message' => __('messages.something_wrong') ?? 'Unable to create branch.',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    {
        Log::info('Branch update invoked', [
            'route' => request()->path(),
            'id' => $id,
            'input' => $request->except(['feature_image'])
        ]);
        ...
        try {
            ...
            if (!empty($request->address) && is_string($request['address'])) {
                ...
                if (json_last_error() === JSON_ERROR_NONE && is_array($decodedAddress)) {
                    ...
                    $clean = Arr::only($decodedAddress, $allowed);
                    if (!empty($clean)) {
                        $addressRelation = $query->address();
                        if ($addressRelation->exists()) {
                            $addressRelation->update($clean);
                        } else {
                            $addressRelation->save(new Address($clean));
                        }
                    }
                }
            }
            ...
            Log::info('Branch update success', ['branch_id' => $query->id]);
            return response()->json(['message' => $message, 'status' => true], 200);
        } catch (\Throwable $e) {
            ...
            return response()->json([
                'status' => false,
                'message' => __('messages.something_wrong') ?? 'Unable to update branch.',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    {
        // Do not remove the employee from other branches if the same employee wants to be assigned to multiple branches

        // Remove only the employees from this branch that are not in the new list
        $currentEmployeeIds = $id->branchEmployee()->pluck('employee_id')->toArray();
        $newEmployeeIds = $request->users ?? [];

        // Delete only those employees who are currently assigned to this branch but not in the new list
        $id->branchEmployee()->whereNotIn('employee_id', $newEmployeeIds)->delete();

        // Add new employees who are not already assigned to this branch
        $employeesToAdd = array_diff($newEmployeeIds, $currentEmployeeIds);

        $employees = [];
        foreach ($employeesToAdd as $emp_id) {
    ...
    {
        $user = Auth::user();
        if ($user && isset($user->branch) && isset($user->branch->branch_id)) {
            $data = Branch::where('id', $user->branch->branch_id)->with('address')->first();
