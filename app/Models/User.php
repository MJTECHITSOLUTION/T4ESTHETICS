...
class User extends Authenticatable implements HasMedia, MustVerifyEmail
{
    ...
    {
        $pathFromDb = $this->attributes['profile_image'] ?? null;
        if (!empty($pathFromDb)) {
            // when stored on public disk, expose via storage symlink
            return asset('storage/' . ltrim($pathFromDb, '/'));
        }

        // fallback to media library if legacy data exists
