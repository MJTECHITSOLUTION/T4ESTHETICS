<template>
  ...
  <div class="d-flex justify-content-between align-items-center">
    ...
    <span class="badge bg-primary">
      <strong>{{ formatTransactionType(bookingTransaction.transaction_type) }}</strong>
    </span>
  </div>
  <div class="d-flex justify-content-between align-items-center my-2">
    <span>Montant du service</span>
    <span>
      <strong>{{ formatCurrencyVue(servicesTotal) }}</strong>
    </span>
  </div>
  <div class="d-flex justify-content-between align-items-center">
    <span>Montant du produit</span>
    <span>
      <strong>{{ formatCurrencyVue(productTotal) }}</strong>
    </span>
  </div>
  <div class="d-flex justify-content-between align-items-center my-2">
    <span>Montant du forfait</span>
    <span>
      <strong>{{ formatCurrencyVue(packageTotal) }}</strong>
    </span>
  </div>
  <div class="d-flex justify-content-between align-items-center" v-for="(tax, index) in bookingTransaction.tax_percentage" :key="index">
    ...
    <template v-else>
      ...
      <span>
        <strong>{{ formatCurrencyVue(tax.tax_amount) }}</strong>
      </span>
    </template>
  </div>
  <div class="d-flex justify-content-between align-items-center">
    <span>Montant du pourboire</span>
    <span>
      <strong>{{ formatCurrencyVue(bookingTransaction.tip_amount) }}</strong>
    </span>
  </div>
</div>
...
<div v-if="couponDiscount > 0" class="form-group d-flex align-items-center justify-content-between">
  <label for="">Remise du coupon :</label>
...
<div class="py-3">
  ...
  <div>
    <h6><i class="fa-regular fa-clock"></i> Réservé le {{ bookingData.created_at }}</h6>
  </div>
  <div class="">
    <h6><i class="fa-regular fa-user"></i> Réservé par {{ bookingData.created_by_name }}</h6>
  </div>
  <div class="mt-4">
    <h6><i class="fa-regular fa-clock"></i> Mis à jour le {{ bookingData.updated_at }}</h6>
  </div>
  <div class="">
    <h6><i class="fa-regular fa-user"></i> Mis à jour par {{ bookingData.updated_by_name }}</h6>
  </div>
</div>
